...
import { FileText, HelpCircle, Image as ImageIcon, LayoutTemplate } from 'lucide-react';
...
export default async function CmsPage() {
    ...
    const [sectionsResult, bannersResult, documentsResult, faqsResult] = await Promise.all([
        supabase
            .from('cms_sections')
            .select(
        ...
        supabase
            .from('cms_documents')
            .select('id, kind, locale, title, version, effective_from, is_published, updated_at')
            .order('kind'),
        supabase
            .from('cms_faqs')
    ...
    if (sectionsResult.error) {
        return (
            <>
                ...
                <Card>
                    <ErrorState title="Could not load storefront content" message={sectionsResult.error.message} />
                </Card>
            </>
        );
    }
    ...
    return (
        <>
            ...
            <InlineNotice tone="info" className="mb-4">
                Sections render in the order below. A rail with no eligible items is skipped automatically, so
                customers never see an empty heading.
            </InlineNotice>

            <Tabs defaultValue="home">
                <TabsList>
                    <TabsTrigger value="home">Home sections ({sections.length})</TabsTrigger>
                    <TabsTrigger value="banners">Banners ({banners.length})</TabsTrigger>
                    <TabsTrigger value="policies">Policies ({documents.length})</TabsTrigger>
                ...
                <TabsContent value="home" className="space-y-3">
                    {sections.length === 0 ? (
                        <Card>
                            <EmptyState icon={LayoutTemplate} title="No home sections configured" />
                        </Card>
                    ) : (
                        sections.map((section) => {
                            const days = section.valid_days_of_week ?? [];
                            const scheduled =
                                days.length < 7 || section.valid_from_time || section.starts_at || section.ends_at;
                            ...
                            return (
                                <Card key={section.id}>
                                    ...
                                    <CardContent className="flex flex-wrap gap-x-6 gap-y-1.5 text-[12.5px] text-ink-muted">
                                        <span>
                                            Key <span className="font-mono text-ink">{section.section_key}</span>
                                        </span>
                                        <span>
                                            Layout <span className="text-ink">{humanise(section.layout)}</span>
                                        </span>
                                        <span>
                                            Shows up to <span className="tnum text-ink">{section.item_limit}</span> items
                                        </span>
                                        {section.action_label ? (
                                            <span>
                                                CTA <span className="text-ink">{section.action_label}</span>
                                            </span>
                                        ) : null}
                                        {scheduled ? (
                                            <span>
                                                ...
                                                <span className="text-ink">
                                                    {days.length < 7 ? days.map((day) => DAY_LABELS[day]).join(', ') : 'daily'}
                                ...
                                return (
                                    <Card key={banner.id} className="overflow-hidden">
                                        ...
                                        <CardContent className="pt-3">
                                            <p className="text-[13.5px] font-medium text-ink">{banner.title ?? 'Untitled'}</p>
                                            {banner.subtitle ? (
                                                <p className="mt-0.5 text-[12.5px] text-ink-muted">{banner.subtitle}</p>
                                            ) : null}
                    ...
                    {documents.map((document) => (
                        <Card key={document.id}>
                            <CardToolbar
                                ...
                                action={
                                    <Badge tone={document.is_published ? 'positive' : 'neutral'}>
                ...
                <TabsContent value="faqs" className="space-y-3">
                    {faqs.length === 0 ? (
                        ...
                        Object.entries(
                            ...
                            <Card key={category}>
                                <CardToolbar title={humanise(category)} description={`${items.length} question(s)`} />
                                    ...
                                    {items.map((faq) => (
                                        <div key={faq.id} className="py-3 first:pt-0 last:pb-0">
                                            <p className="flex items-start gap-2 text-[13.5px] font-medium text-ink">
                                                ...
                                                {!faq.is_published ? (
                                                    <Badge tone="neutral" className="shrink-0 px-1.5 py-0">
                                                        Draft
                                                    </Badge>
                                                ) : null}
                                            </p>
