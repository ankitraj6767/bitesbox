...
export function normaliseError(error: unknown): NormalisedError {
  if (!error) {
    return {
      ...
      message: fallback(ERROR_CODES.UNKNOWN),
    };
  }

  // Edge Function errors can arrive as a raw JSON string. Parse the standard
  // envelope so actionable backend messages are not hidden by a generic toast.
  if (typeof error === "string") {
    try {
      return normaliseError(JSON.parse(error) as unknown);
    } catch {
      return { code: ERROR_CODES.UNKNOWN, message: error };
    }
  }
  ...
  if (typeof error === "object" && error !== null) {
    ...
    if (candidate.error?.code) {
      return {
        ...
        details: candidate.error.details ?? candidate.error.detail,
      };
    }
...
function fallback(code: string): string {
  return (
    ERROR_FALLBACK_MESSAGES[code] ?? "Something went wrong. Please try again."
  );
}
